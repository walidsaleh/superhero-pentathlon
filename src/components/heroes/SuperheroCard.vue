...
<template>
  <div
    class="superhero-card group relative bg-white dark:bg-gray-900 rounded-lg shadow-lg dark:shadow-white/5 overflow-hidden hover:shadow-xl dark:hover:shadow-white/10 transition-shadow duration-200"
  >
    <div class="h-48 flex items-center justify-center bg-gray-50 dark:bg-gray-800">
      <img
        :src="hero.image"
        :alt="hero.name"
      ...
      />
      <div
        class="absolute inset-0 bg-white dark:bg-sky-950 opacity-0 group-hover:opacity-70 transition-opacity duration-200"
      />
      <div
        class="absolute inset-0 opacity-0 group-hover:opacity-100 transition-opacity duration-200 flex items-center justify-center space-x-4"
      >
        <button
          @click="$emit('edit', hero)"
          class="p-2 bg-sky-600 dark:bg-sky-500/30 hover:bg-sky-700 dark:hover:bg-sky-500/90 rounded-full text-white transition-colors duration-200 cursor-pointer"
        ...
        >
          <ArrowPathIcon v-if="loadingDelete === hero.id" class="w-5 h-5 animate-spin" />
