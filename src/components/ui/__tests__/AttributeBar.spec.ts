import { BoltIcon } from '@heroicons/vue/24/solid' // Import at least one icon for component rendering check
...
// Configure vue-i18n for testing with basic translations
...
describe('AttributeBar', () => {
  // Helper function to mount the component with default props and i18n
  ...
  describe('compact prop', () => {
    it('should render the compact template when compact is true', async () => {
      ...
      // Snapshot of the component's HTML structure in compact mode
      ...
      expect(wrapper.find('.sr-only').text()).toMatchInlineSnapshot('"Agility"') // Check the screen reader label
    ...
    it('should render the non-compact template when compact is false or undefined', () => {
      ...
      const wrapperCompactUndefined = mountComponent({ value: 4, type: 'weight' }) // 'compact' prop is omitted, should default to non-compact
